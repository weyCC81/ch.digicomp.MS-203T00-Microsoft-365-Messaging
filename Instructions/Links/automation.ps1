...
# --- Certificate
#$mycert = New-SelfSignedCertificate -FriendlyName "Digicomp EXO Litigation Hold" -Subject $env:Computername -CertStoreLocation "cert:\LocalMachine\My" -NotAfter (Get-Date).AddYears(6) -KeySpec KeyExchange
#mkdir C:\DIGI\Scripts\MailboxLitigationHold
#$mycert | Export-Certificate -FilePath C:\DIGI\Scripts\MailboxLitigationHold\mycert.cer
#Get-ChildItem "cert:\LocalMachine\My" | Where {$_.FriendlyName -eq "DIGI EXO Powershell"}
...
Connect-ExchangeOnline -CertificateThumbprint E2A57AB8B90570394DE0BA8F63649E4A3DB66EBX -AppId 4ef5a924-9c03-485d-8d7e-7312097bd032 -Organization $TenantName
# Run as Admin for Access to Computer Certificate Store


# --- Get-UnifiedGroup
# with CAB no Microsoft 365 Groups available!
Get-UnifiedGroup
