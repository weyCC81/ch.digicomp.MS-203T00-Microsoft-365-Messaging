...
**Note**: During the next series of task you will be moving to the **Anti-malware** page in the **Office 365 Security and Compliance center**. Protection services will no longer reside in the EAC for Exchange Online.
...
4. In the **Office 365 Security & Compliance center**, in the left-hand navigation pane, select **Threat Management** and then select **Policy**.
...
19. On the **Review your settings** page, review the settings that you just configured. If anything needs to be corrected, select the corresponding **Edit** option to make the necessary fix. If everything is correct, select the **Create this policy** button at the bottom of the page. <br/>

**Note**: A **Security & Complaince** window will appear with a message that indicates your organization settings need to be updated. Select **Yes** to continue.
...
3. In the **Office 365 Security & Complaince admin center**, in the left-hand navigation pane, select **Threat Management** and then select **Policy**.
...
5. In the **Default spam filter policy (always ON)** pane, you will be presented a variety of options on how you would like spam to be handled and what rating will be triggered depending on the severity of the spam. The following steps will guide you through these settings so that you can update them per Adatum's requirements.
...
9. Select the **International spam** drop-down arrow and update the following settings:
	...
	**Note:** This section allows you to automatically tag as spam those messages sent from countries that are blocked, as well as messages written in a specific language.
	...
	- Filter email messages written in the following languages: perform the following steps:
		- Select **Edit**
		...
		- Type the letter **"A"** in the **Language** field to display the list of languages
		- Select any language you want to restrict
		- If you want to restrict an additional language, repeat the prior two steps (you can restrict as many languages as you wish)
